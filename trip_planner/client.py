...
class TripPlanner(object):
    ...
    def __validate_bart_station(self, stop_tag):
    ...
    def leg_list(self):
        legs = OrderedDict()
        _legs = self.db_session.query(Leg, LegInclude).join(LegInclude, Leg.id == LegInclude.leg_id)
    ...
    def leg_delete(self, leg_id):
        ...
        for includes in self.db_session.query(LegInclude).filter_by(leg_id=leg_id):
    ...
    def trip_list(self):
        trips = OrderedDict()
        _trips = self.db_session.query(Trip, TripLeg).join(TripLeg, TripLeg.trip_id == Trip.id)
